...
class SetupProcess:
    ...
    def audio_check(self):
        return os.path.exists("Siosk_en/assets/audio") and os.path.isdir("Siosk_en/assets/audio")
    ...
    def setup_process(self):
        download.download_file(file="conversation_en.json", save_dir="Siosk_en/package/")
        if self.OS == "Windows":
        ...
        try:
            from SioskServer_en.server import process
            sys.path.append(os.path.dirname(os.path.abspath(os.path.dirname(__file__))))
            process()    
        except:
            unpredictable_problem()
